...
export async function verifyPairChoiceScreen(
  ...
  debug('Verifying /pair choice screen');
  ...
  const isDisabled = await client.executeScript(`
    var btn = document.querySelector('#set-needs-mobile');
    return btn ? btn.disabled : null;
  `);
  if (isDisabled !== true) {
    debug('Warning: Continue button was not disabled before radio selection');
  }
  ...
  const hasMobileLabel = await findElementBySelectors(
    client,
  ...
  debug('/pair choice screen verified successfully');
}
