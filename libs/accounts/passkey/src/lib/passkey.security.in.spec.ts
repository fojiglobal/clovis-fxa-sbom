...
describe('Passkey Security Tests', () => {
  ...
  describe('Challenge Security', () => {
    ...
    it('generates 100 unique challenges each with >= 32 bytes of entropy', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        const challenge = await challengeManager.generateRegistrationChallenge({
          uid: fakeUid(),
        });
    ...
    it('validateChallenge physically deletes the Redis key on first use', async () => {
      const challenge = await challengeManager.generateRegistrationChallenge({
        uid: fakeUid(),
      });

      await challengeManager.validateChallenge(challenge, 'registration');
    ...
    it('a registration challenge is rejected when presented as an authentication challenge and remains unconsumed', async () => {
      ...
      expect(
        await challengeManager.validateChallenge(challenge, 'authentication')
      ).toBeNull();

      const correct = await challengeManager.validateChallenge(
        challenge,
        'registration'
      );
